...
class DiagGaussianDistribution(Distribution):
    ...
    def pre_log_prob(self, actions: th.Tensor, only_dist_params = False) -> th.Tensor:
        
        if only_dist_params:
            return self.distribution.mean, self.distribution.scale
        
        log_prob = self.distribution.log_prob(actions)
        
        '''
        if sum(log_prob.flatten() > 0) > 0:
            print('@distrib-prelog:', log_prob)
            print('distrib is ', self.distribution)
            print('loc (mean):', self.distribution.mean, ', scale:', self.distribution.scale)
            print('acts input:', actions)
            
            # possible if distribution pdf at actions is POSITIVE.. (this is ok)
            # cause log_std VERY SMALL!
            
            raise ValueError()'''
            
        return log_prob, self.distribution.mean, self.distribution.scale #self.distribution.log_prob(actions)
...
class SquashedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def log_prob(self, actions: th.Tensor, gaussian_actions: Optional[th.Tensor] = None) -> th.Tensor:
        ...
        if gaussian_actions is None:
            ...
            gaussian_actions = TanhBijector.inverse(actions) # u = tanh^-1(a)
        
        '''
        print('@distrib-Squashed l222, action:', actions.shape, actions)
        print('gauss:', gaussian_actions.shape, gaussian_actions)
        '''
        # Log likelihood for a Gaussian distribution
        log_prob = super().log_prob(gaussian_actions)
        # this should implement log \mu(u|s) where \mu ~ Normal(), value \in (-inf, 0)
        
        # Squash correction (from original SAC implementation)
        # this comes from the fact that tanh is bijective and differentiable
        log_prob -= th.sum(th.log(1 - actions**2 + self.epsilon), dim=1) 
        # becomes POSITIVE LARGE if either log_prob_u v small negative or 
        # 2nd term very large negative.. since actions~tanh(u), log(1 - tanh(u)^2) is unbounded below
        
        # becomes NEGATIVE if either 1st term v large negative or 2nd term v small negative
        
        #print('log pi(a|s), becomes negative when?', log_prob)
        #print('---')
        
        return log_prob
    
    def log_prob_u(self, only_dist_params = False) -> th.Tensor:
        
        if only_dist_params:
            return super().pre_log_prob(self.gaussian_actions, only_dist_params)
        
        log_prob = super().log_prob(self.gaussian_actions)
        
        try:
            pre_sum_dim_logProb, mean_acts, log_std = super().pre_log_prob(self.gaussian_actions)
        except:
            pre_sum_dim_logProb, mean_acts, log_std = None
        
        return self.gaussian_actions, log_prob, pre_sum_dim_logProb, mean_acts, log_std
    ...
    def log_prob_from_params(self, mean_actions: th.Tensor, log_std: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        ...
        # u = Normal(mean, log_std)
        
        log_prob = self.log_prob(action, self.gaussian_actions) # log_prob_a        
        # u = self.gaussian_actions
        # a = action = tanh(u)
