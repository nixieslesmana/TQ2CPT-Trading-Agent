...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def learn(
        ...
        print('@off-policy, num_timesteps:', self.num_timesteps, '/', total_timesteps)
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if self.num_timesteps > 0 and self.num_timesteps > self.learning_starts:
                ...
                if gradient_steps > 0:
                    logger_dict = self.train(batch_size=self.batch_size, gradient_steps=gradient_steps, logger_dict = logger_dict)
        
        #print('@off-policy l375, logger_dict:', logger_dict)
        to_write = []
        for key_ in logger_dict.keys():
            to_write += [key_ + ': ' + str(logger_dict[key_]) + '\n']
            '''
            if key_ not in ['crash_iters', 'n_updates'] and len(logger_dict[key_]) > 0:
                plt.figure()
                plt.plot(logger_dict[key_], label = key_)
                plt.legend()
                plt.savefig('./output/'+ key_ + '_s{}_{}.png'.format(self.seed, self.name[:-12]), dpi=100)
                plt.close()'''

        #runID = datetime.now().strftime("%d%m%Y%H%M%S")            
        filename = './output/logs_s{}_{}.txt'.format(self.seed, self.name)#, runID)
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            #print('@off-pol, here2')
            #print('._last_obs:', list(self._last_obs[0]))
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            '''
            print('@off-policy-algo, l617 -- AFTER env.step()')
            #print('prev_obs:', list(env.state[0]))
            print('actions:', list(actions[0]))
            print('new_obs:', list(new_obs[0])) #, new_obs.shape)
            '''
            # AFTER UPDATE n=0,
            # new_obs in dummy = obs (env.state)
            # maybe 'actions' fault
            
            
            #print('rewards:', rewards)    
            #print('@train-ENV, day:', self.day)
            #print('done condition:', len(self.df.index.unique())-1) # 1761
