...
def compute_CPT(tensor, sort = True, B=b_):
    ...
    try:
        ...
        CPT_val_pos = np.dot(u_pos, dwF_pos)
    except:
        CPT_val_pos = 0
    ...
    try:
        ...
        CPT_val_neg = np.dot(u_neg, dwF_neg)
    except:
        CPT_val_neg = 0
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                # TRANSFORM WITH r + gamma * next_quantiles!!!
            ...
            '''
            current_quantiles.dim = (batchSize, 3, 25)???
            If so, reshape in l.426 --> (batchSize, 75) --> sorted???? ruin the "taus" identity?
            
            @huber, taus used are 25-atoms
            so, actor_quantiles using critic.forward(feat(s, a)) ~ criticPred
            with output (batch, 25, 3).
            [!] Should change:
                - actor_quantiles.flatten() to actor.quantiles.mean(dim=1), out: (, 25) fro in: (3, 25)
                - compute_CPT(out), with CPT_WEIGHTING
                - set critic_mode = None
            [?] Why not use target? SAC does right? Check original sourcecode TQC:
                - also use critic, perhaps cz critic_target is NOT trainable
            '''
            ...
            if self.actor_mode == 'cpt':
                ...
                qf_pi = th.stack([compute_CPT(qf_n, sort = False) for qf_n in th.unbind(actor_quantiles.mean(1), dim=0)]).to(th.float64).mean(0)#.item()
