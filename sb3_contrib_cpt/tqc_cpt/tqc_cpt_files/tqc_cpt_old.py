...
def cpt_weighting(F, pos, rho1 = rho1, rho2 = rho2): 
    ...
    # @Tversky: rho1 = .61, rho2 = .69 lambda=2.25, alpha=0.88;
...
def compute_prelec(tensor, total_quantiles, sort=True, B = 1.0):
...
def compute_CPT(tensor, sort = True, B=b_):
    ...
    try:
        ...
        CPT_val_neg = 0
    # try:
    #     CPT_val_pos = np.nansum((np.multiply(vectorised_utility(XList[np.array(pos_mask)], True, alpha_, lambda_, B), dF_i_pos)))
    # except:
    #     CPT_val_pos = 0
    
    # dF_i_neg = neg_diff_proba - neg_proba 
    # try:
    #     CPT_val_neg = np.nansum((np.multiply(vectorised_utility(XList[np.array(neg_mask)], False, alpha_, lambda_, B), dF_i_neg)))
    # except:
    #     CPT_val_neg = 0
...
# def compute_prelec(tensor, sort=True, B = 1.0, n_target_quantiles=25):
...
#     # i = np.arange(1, m+1)
#     # taus = i/m
#     vectorised_prob_func = np.vectorize(prelec_weighting)
#     vectorised_utility = np.vectorize(utility)
...
#     q = len(XList)
#     k = ceil(q/n_target_quantiles)
#     midpoint = 1/n_target_quantiles
#     midpoint /= 2
#     taus = th.linspace(start = midpoint, end = 1-midpoint, steps = n_target_quantiles)
#     taus = taus.repeat_interleave(k)
#     taus = taus[:m]

#     proba = vectorised_prob_func(taus)
#     diff_proba = np.roll(proba, 1)
#     diff_proba[0] = np.nan
 
#     dF_i = diff_proba - proba
#     dF_i_pos = dF_i[np.array(pos_mask)]
#     CPT_val_pos = np.nansum((np.multiply(vectorised_utility(XList[np.array(pos_mask)], pos=True), dF_i_pos)))
    

#     dF_i_neg = dF_i[np.array(neg_mask)]
#     CPT_val_neg = np.nansum((np.multiply(vectorised_utility(XList[np.array(neg_mask)], pos=False), dF_i_neg)))
...
#     i = np.arange(1, m+1)
#     vectorised_prob_func = np.vectorize(cpt_weighting)
#     vectorised_utility = np.vectorize(utility)
...
#     pos_proba = vectorised_prob_func(1-i/m, pos=True)
#     neg_proba = vectorised_prob_func(i/m, pos=False)
#     pos_diff_proba = np.roll(pos_proba, 1)
#     pos_diff_proba[0] = np.nan
#     neg_diff_proba = np.roll(neg_proba, 1)
#     neg_diff_proba[0] = np.nan
    
        
#     dF_i_pos = pos_diff_proba - pos_proba
#     dF_i_pos = dF_i_pos[np.array(pos_mask)]

#     CPT_val_pos = np.nansum((np.multiply(vectorised_utility(XList[np.array(pos_mask)], pos=True), dF_i_pos)))
    

#     dF_i_neg = neg_diff_proba - neg_proba
#     dF_i_neg = dF_i_neg[np.array(neg_mask)]
#     CPT_val_neg = np.nansum((np.multiply(vectorised_utility(XList[np.array(neg_mask)], pos=False), dF_i_neg)))
#     CPT_val = th.tensor(CPT_val_pos + CPT_val_neg)
...
class TQC_CPT(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            truncated_quantiles, _ = th.sort(current_quantiles.reshape(batch_size, -1))
            truncated_quantiles = truncated_quantiles[:, :n_target_quantiles]

            # Compute critic loss, not summing over the quantile dimension as in the paper.
            # critic_loss = cpt_quantile_huber_loss(current_quantiles, target_quantiles, sum_over_quantiles=False, lambda_= self.lambda_ , p=self.p)
            # critic_loss = prelec_quantile_huber_loss(truncated_quantiles.flatten(), target_quantiles.flatten(), sum_over_quantiles=False, a=self.a, n_target_quantiles=self.critic.quantiles_total)
            ...
            # Compute actor loss
            actor_quantiles = self.critic(replay_data.observations, actions_pi)
            if self.actor_mode == 'cpt':
                qf_pi = compute_CPT(actor_quantiles.flatten())
