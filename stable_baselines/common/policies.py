...
from stable_baselines.common.tf_layers import conv, linear, conv_to_fc, lstm, fc
...
class MlpPolicyIntrinsicReward(object):
    def __init__(self, sess, ob_space, ac_space, nbatch, nsteps, reuse=False): #pylint: disable=W0613
        ...
        #print('@mlp intr:', nbatch, ob_space.shape)
        actdim = ac_space.shape[0]
        #print("actdim:", actdim, ac_space.shape)
        ...
        with tf.compat.v1.variable_scope('intrinsic', reuse=reuse):
            ###CHANGE TO X_NX: X_ALL = tf.compat.v1.placeholder(tf.float32, (None,) + ob_space.shape, name='Ob_all') #obs
            A_ALL = tf.compat.v1.placeholder(tf.float32, [None, actdim], name='Ac_all')  # obs
            INPUT = tf.concat([X, A_ALL], axis=1)
            activ = tf.tanh
            ...
            r_in0 = tf.tanh(fc(h2, 'r_in', 1))[:,0]    #r_in(s,a): estimated by eta, here the params eta refers to this layer of NN
        ...
        def intrinsic_reward(ob, ac, *_args, **_kwargs):    ###INPUT INCLUDE OB_NX
            ...
            #else:
            # r_in = .. X:ob
            r_in = sess.run(r_in0, {X:ob, A_ALL:ac})    ###CODE TO EXPEND AND INCLUDE X_NX
            #print("r_in:", r_in)
            return r_in

        def proba_step(ob, state=None, mask=None):
            return sess.run(self.policy_proba, {self.obs_ph: obs})
...
class MlpPolicyNew(object):
    def __init__(self, params, ob_space, ac_space, nbatch, nsteps): #pylint: disable=W0613
        ob_shape = (nbatch,) + ob_space.shape
        #print('@mlp new:', nbatch, ob_space.shape)
        X = tf.compat.v1.placeholder(tf.float32, ob_shape, name='Ob') #obs
        with tf.name_scope('policy_new'):
            activ = tf.tanh
            #print("X:", X, params['policy/pi_fc1/w:0'],params['policy/pi_fc1/b:0'] )
            #print("fc2:", params['policy/pi_fc2/w:0'], params['policy/pi_fc2/b:0'])
            #print("fc3:", params['policy/pi/w:0'], params['policy/pi/b:0'])
            h1 = activ(tf.compat.v1.nn.xw_plus_b(X, params['policy/pi_fc1/w:0'], params['policy/pi_fc1/b:0']))
...
class MlpPolicyIntrinsicInnovationReward(object):
    def __init__(self, sess, ob_space, ac_space, nbatch, nsteps, reuse=False): #pylint: disable=W0613
        ...
        with tf.compat.v1.variable_scope('policy', reuse=reuse):
            ...
            pi = fc(h2, 'pi', actdim, init_scale=0.01)
        with tf.compat.v1.variable_scope('intrinsic', reuse=reuse):
            X_NX = tf.compat.v1.placeholder(tf.float32, (None,) + ob_space.shape, name='Ob_all') #obs
            A_ALL = tf.compat.v1.placeholder(tf.float32, [None, actdim], name='Ac_all')  # obs
            INPUT = tf.concat([X, A_ALL, X_NX], axis=1)
            activ = tf.tanh
            h1 = activ(fc(INPUT, 'intrinsic_fc1', nh=64, init_scale=np.sqrt(2)))
            #h1 = activ(fc(X, 'intrinsic_fc1', nh=64, init_scale=np.sqrt(2)))
            h2 = activ(fc(h1, 'intrinsic_fc2', nh=64, init_scale=np.sqrt(2)))
...
_policy_registry = {
    ActorCriticPolicy: {
        ...
        "MlpPolicyIntrinsicReward":MlpPolicyIntrinsicReward,
